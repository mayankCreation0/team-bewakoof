...
const url="https://bewakoof-app.onrender.com/mens";
...
const Firstdisplay =async()=>{
    let data = await get('https://bewakoof-app.onrender.com/mens');
    display(data);
}
Firstdisplay();
 ...
 const Applyfilter =async ()=>{
  checkforcolor();
   ...
   if(category!=""){
    const url=`https://bewakoof-app.onrender.com/mens?category=${category}`
    ...
    const url=`https://bewakoof-app.onrender.com/mens?types=${type}`
    let data = await get(url);
    if(color!=""){
        data = data.filter((el)=>{
            return el.color===color;
    ...
    return;
     ...
     console.log(color);
    const url=`https://bewakoof-app.onrender.com/mens?color=${color}`
    let data = await get(url)
    console.log(data);
     if(size!=""){
        data = data.filter((el)=>{
    ...
    const url=`https://bewakoof-app.onrender.com/mens?size=${size}`
    let data = await get(url);
     if(rating!=""){
        // const ratingurl= await get(`https://bewakoof-app.onrender.com/mens`);
       if(rating==="highRating"){
...
document.getElementById("sort_price").addEventListener("change",async()=>{
   ...
   let url="https://bewakoof-app.onrender.com/mens"
   let data = await get(url);
   if(curr==="highToLow"){
...
function boxShow(data){
    ...
    data.map((el)=>{
        ...
        div.addEventListener("click",()=>{
            ...
            const urlfl=`https://bewakoof-app.onrender.com/mens?q=${flag}`
            document.getElementById("suggestions").innerHTML="";
            get(urlfl);
        })
...
function debounce(fn,delay){
 ...
 async function show(){
    ...
    const url=`https://bewakoof-app.onrender.com/mens?q=${search}&_limit=5`;
    const res=await fetch(url);
    const data= await res.json();
    boxShow(data);
   }
